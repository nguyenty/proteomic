...
reg <- lm(log(sd_depleted)~log(sd_whole))
par(cex=.8)
plot(log(sd_whole), log(sd_depleted), main ="Scatter plot of Log(Standard Error)" )
abline(reg)
