...
Function to fit a linear mixed model for each spot, with fixed effect is Cy, RFI Line and random effect is animal.
...
Proportion of protein spots whose standard error in the depleted sample larger than that one  in the whole sample.
<<>>=
...
Consider a Wilcoxon signed-rank test H0: The standard errors are the same in depleted samples vs. whole samples, H1: the standard errors are significantly higher overall in depleted samples vs. non-depleted samples.  The first test using the non-transformed standard errors has p-value = 0.6575. The second test using the log-transformed standard errors has p-value = 0.1706. 
